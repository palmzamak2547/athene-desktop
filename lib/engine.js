...
// Locate the Athene CLI bundle (dist/cli.js). Resolution order — the BUNDLED
// engine (shipped inside the app) is always preferred; the C:/dev path is only
// a last-ditch DEV fallback, so a blank machine resolves to a real bundled file.
//   1. ATHENE_CLI env (explicit override)
//   2. packaged: bundled via extraResources (resources/athene/dist/cli.js)
//   3. dev: vendored engine in-repo        (<app>/vendor/athene/dist/cli.js)
//   4. dev sibling repo                    (<app>/../athene/dist/cli.js)
//   5. a last-ditch dev default            (C:/dev/athene/dist/cli.js)
export function resolveAtheneCli({ resourcesPath, appPath } = {}) {
  ...
  if (appPath) tries.push(path.join(appPath, "vendor", "athene", "dist", "cli.js"));
